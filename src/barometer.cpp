...
namespace zestimation {
  ...
  void Barometer::calibrate(float pressure)
  {
      static float _groundPressure;
      _groundPressure += (float)pressure / _calibrationIters;
  ...
  bool Barometer::update(float pressure)
  {
      ...
      // is noise
      float pressure_filt  = (pressure * 100);
      _pressure = _lpf.update(pressure_filt);
      // if required, calibrate baro
      if (_iters < _calibrationIters)
      {
        Barometer::calibrate(pressure_filt);
        _iters += 1;
      }
      else {
        calibrating = false;
